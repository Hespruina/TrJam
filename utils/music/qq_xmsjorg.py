...
class QQMusicSearch:  # 类名保留兼容性
    ...
    def _is_vip_song(self, mp3_url):
        """判断是否为 VIP 歌曲（适用于 QQ 音乐）"""
        if not mp3_url or "http" not in mp3_url:
            return True
        ...
        try:
            ...
            if any(t in content_type for t in ["audio/", "video/", "application/octet-stream"]):
                # 但若文件太小（<10KB），可能是无效响应
                return content_length < 1024 * 10

            return True  # 其他类型视为 VIP
    ...
    def search(self, keyword, page=1):
        ...
        :return: dict
        """
        # 动态设置 __tins__ cookie（模拟真实用户）
        sid = int(time.time() * 1000)
        expires = sid + 1800000  # 30分钟后过期
        self.session.cookies.set(
            "__tins__19997613",
            f'{{"sid": {sid}, "vd": 2, "expires": {expires}}}'
        )
        ...
        referer = f"http://xmsj.org/?name={urllib.parse.quote(keyword)}&type=qq"
        ...
        # xmsj.org 返回结构：成功时 code=200，data 是列表
