...
async def handle_like_command(context: BotContext, args: list, user_id: str, group_id: str, **kwargs) -> CommandResponse:
    ...
    create_monitored_task(
        process_like_request(context, user_id, group_id, account_id),
...
async def process_like_request(context: BotContext, user_id: str, group_id: str, account_id: int = None):
    """
    在后台处理点赞请求的操作
    ...
    :param user_id: 用户 ID
    :param group_id: 群 ID
    :param account_id: 账号 ID（parallel 模式下使用）
    """
    try:
        # 持续点赋试验，直到失败为止
        success_count = 0
        max_attempts = 60  # 最大尝试次数保持 60 次以检测假成功情况
        ...
        for i in range(max_attempts):
            ...
            # 检查 API 返回结果
            if not result:
                logger.error(f"名片赞 API 调用失败，无返回结果")
                break
            
            # 根据 data 中的 status 字段判断是否成功
            if result.get("data") and result["data"].get("status") == "ok":
                success_count += 1
                logger.info(f"为用户 {user_id} 第 {success_count} 次名片赞操作成功（每次 10 个）")
                
                # 如果还没达到最大尝试次数，短暂延迟后继续下一次
                if i < max_attempts - 1:
                    await asyncio.sleep(1)  # 短暂延迟 1 秒
            else:
                logger.info(f"为用户 {user_id} 名片赞操作失败，停止点赞。返回结果：{result}")
                break
        
        # 发送最终结果给用户，将成功次数乘以 10
        if success_count >= 60:
            # 如果成功次数达到 60 次，说明是假成功情况，提示用户加好友重试
            message = "❌ 请先添加机器人为好友再重试"
        elif success_count > 0:
            total_likes = success_count * 10  # 每次点 10 个赞，所以总数要乘以 10
